...
kernel void backprojectPixelDriven2DCL(
	...
	float normalizationFactor = maxThetaIndex / M_PI_F;
	int idx = mad24(y,gridSizeX,x);
	grid[idx] = 0;
	
	for(int b=0; b<maxThetaIndex; ++b){
		// compute theta [rad] and angular functions.
		float theta = deltaTheta * b;
		float cosTheta = cos(theta);
		float sinTheta = sin(theta);

		float2 a = {focalLength * cosTheta, focalLength * sinTheta};
		float2 p0 = {-maxT / 2.f * sinTheta, maxT / 2.f * cosTheta};
		
		// compute two points on the line through t and theta
