...
public class FanBeamSparseViewReconTest {
	...
	public static void main (String [] args) throws Exception{
		...
		String imgPath;
		String saveFolderPath = "C:\\Tasks\\FAU4\\SparseView\\testData_d1\\";
		String reconFolderPath = "C:\\Tasks\\FAU4\\SparseView\\recons\\";
		String reconFbpPath;
		String artifactPath;
		String recon3DPath;
		...
		FanBeamSparseViewReconTest obj = new FanBeamSparseViewReconTest();
		...
		for(int patient = 3; patient <= 3; patient ++)
		{
			if(patient == 4)
				continue;
			path3 = saveFolderPath + patient + "\\";
			recon3DPath = reconFolderPath + "reconFBP" + patient + ".tif";
			...
			for(int idx = 0; idx <= 255; idx ++)
			{
				...
				recon3D.setSubGrid(idx, (Grid2D)recon.clone());
				
				reconFbpPath = path4 + "data" + idSave + ".tif";
				artifactPath = path4 + "data" + idSave + "_mask.tif";
			...
			path5 = path3 + "\\evaluation\\";
			outPutDir = new File(path5);
			if(!outPutDir.exists()){
		    outPutDir.mkdirs();
			}
			imp = ImageUtil.wrapGrid(recon3D, null);
			IJ.saveAs(imp, "Tiff", recon3DPath);
			System.out.println(" ");
		}
		System.out.println();
