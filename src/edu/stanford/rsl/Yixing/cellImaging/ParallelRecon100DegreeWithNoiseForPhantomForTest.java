...
public class ParallelRecon100DegreeWithNoiseForPhantomForTest {
	...
	public static void main (String [] args) throws Exception{
		...
		ParallelRecon100DegreeWithNoiseForPhantomForTest obj = new ParallelRecon100DegreeWithNoiseForPhantomForTest();
		...
		for(int imgIdx = 0; imgIdx < 512; imgIdx = imgIdx + zs )
		{
			...
			phan = phan3D.getSubGrid(imgIdx);
			phan.getGridOperator().addBy(phan, 0.15f);
			obj.addFOVCircle(phan);
			imp = ImageUtil.wrapGrid(phan, null);
			imgPath = referencePath + imgIdx + ".tif";
			IJ.saveAs(imp, "Tiff", imgPath);
			idSave = imgIdx;
			...
			imp = ImageUtil.wrapGrid(sinogram, null);
			sinoPath = sinogramPath + imgIdx + ".tif";
			IJ.saveAs(imp, "Tiff", sinoPath);
