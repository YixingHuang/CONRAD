...
/**
 * @author Yixing Huang
 * this is a bead phantom, which contains multiple beads with various radius and intensities.
 *
 */
public class BeadPhantom {

	private Grid3D image;
	private boolean init = false;
	private float r0 = 0.8f; //inner cylinder radius
	private int zb, zt;
	private int nx = 0;
	private int ny = 0;
	private int nz = 0;
	...
	private int numNano = 0; //number of gold nano-particles outside the cell
	private int numNano2 = 50; //number of gold nano-particles on the cell wall
	private int numPetal = 0;
	private double[][] ellipsoids;


	public BeadPhantom( int maxI, int maxJ, int maxK, int numCyano, int numNano, int numNano2){
	...
	public BeadPhantom( Grid3D grid, int numCyano, int numNano, int numNano2){
		this.image = grid;
	...
	private void computeNumericalPhantom( ){
		...
		int zb = (int)((0.125 + (Math.random() - 0.5) * 0.05) * this.nz);
		int zt = (int)((0.875 + (Math.random() - 0.5) * 0.05) * this.nz);
		this.zb = zb;
		this.zt = zt;
		double r = 0.8 - Math.random() * 0.2;
		int radius = (int)(r/2.0 * this.nx);
		this.r0 = (float)(r - 0.01 - (float)(Math.random() * 0.02));
//		int radius2 = (int) ( this.r0 / 2.0f * this.nx);
		int rr = radius * radius;
//		int rr2 = radius2 * radius2;
		int rrr;
		int xcent = this.nz / 2;
		int ycent = this.ny / 2;
		float rho = (float)(0.15 + 0.1 * Math.random() * 0.1);
		float rho2 = (float)(1.0 - Math.random() * 0.1);
		for(int k = zb; k < zt; k++){
			for(int i = -radius; i < radius; i++)
			{
				for(int j = -radius; j < radius; j++) {
					rrr = i * i + j * j;
					if(rrr< rr)
						this.image.setAtIndex(i + xcent, j + ycent, k, rho);
//					else if(rrr < rr)
//						this.image.setAtIndex(i + xcent, j + ycent, k, rho2);	
				}
			}
		}
...
//		for(int idx = 0; idx < numCyano + numNano + numNano2 + numPetal + 2; idx ++)
		for(int idx = 0; idx < numCyano + numNano + numNano2 + numPetal; idx ++)
		{
...
//		ellipsoids = new double[numCyano + numNano + numNano2 + numPetal + 2][10];
		ellipsoids = new double[numCyano + numNano + numNano2 + numPetal][10];
...
//		System.arraycopy(cellWall[1], 0, ellipsoids[1], 0, 10);
		...
		while(i < numCyano)
		{
			for(int j = 0; j < 3; j++)
			{
				temp[0][j] = (Math.random() - 0.5) * 1.5 * r;
				temp[0][j + 6] = (Math.random() - 0.5) * Math.PI; 
			}
...
//			if(checkBoundary(temp, 0.01))
//			{
//				System.arraycopy(temp[0], 0, ellipsoids[i + numPetal], 0, 10);
//				i++;
//			}
			System.arraycopy(temp[0], 0, ellipsoids[i + numPetal], 0, 10);
			i++;
...
//			if(checkBoundary(temp, 0.01))
//			{
//				System.arraycopy(temp[0], 0, ellipsoids[i + numPetal + numCyano], 0, 10);
//				i++;
//			}
			System.arraycopy(temp[0], 0, ellipsoids[i + numPetal + numCyano], 0, 10);
			i++;
		...
		while(i < numNano2)
		{
			...
			r3 = (Math.random() - 0.5) * 0.1;
			temp[0][3] = r3;
			temp[0][4] = r3 ;
			temp[0][5] = r3;
			temp[0][9] = 1.5 + (Math.random() - 0.5) * 1;
//			if(checkBoundary(temp, 0.01))
//			{
//				System.arraycopy(temp[0], 0, ellipsoids[i + numPetal + numCyano + numNano], 0, 10);
//				i++;
//			}
//			if(temp[0][9] > 0) {
...
// 				i++;
//			}
			System.arraycopy(temp[0], 0, ellipsoids[i + numCyano + numNano + numPetal], 0, 10);
				i++;
		}
	}
	
	private boolean checkBoundary2(double [][] ellipsoid, double r, double tolerance)
	{
...
//	private boolean checkBoundary(double [][] ellipsoid, double tolerance)
//	{
//
//		double dd = 0;
//		double d;
...
//		d = Math.sqrt(dd);
//		double maxR = Math.max(Math.max(ellipsoid[0][3], ellipsoid[0][4]), ellipsoid[0][5]);
//		if(maxR + d >= this.r0 - tolerance)
//			return false;
//		else if((ellipsoid[0][2] + maxR) * this.nz/2 > this.zt || (ellipsoid[0][2] - maxR) * this.nz/2 < this.zb)
//			return false;
//		else
//			return true;
//	}
