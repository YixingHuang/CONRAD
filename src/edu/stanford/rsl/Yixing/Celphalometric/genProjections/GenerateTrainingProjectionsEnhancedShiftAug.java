...
import edu.stanford.rsl.Yixing.Celphalometric.spacing.SpacingDataLower;
...
public class GenerateTrainingProjectionsEnhancedShiftAug {
...
//		SpacingData sp = new SpacingData();
		SpacingDataLower sp = new SpacingDataLower();
		GenerateTrainingProjectionsEnhancedShiftAug obj = new GenerateTrainingProjectionsEnhancedShiftAug();
		String inputPath = "E:\\CQ500CTData\\UpperHalf\\";
//		String inputPath = "E:\\CQ500CTData\\LowerHalf\\";
		String outputPath = "D:\\Tasks\\FAU4\\Cephalometric\\parallelProjectionsEnhancedLowerAug\\";
...
//			obj.thresholdVol(vol, 900, 3600);
			...
			obj.enhanceRegions(vol, 1500, 400, 1.4f);
			...
			else if(idx == 62 || idx==310) {
				deltaZNum = 100;
//				deltaZNum = 110;
//				deltaZNum = 120;
				obj.imgSizeZ = vol.getSize()[2] - deltaZNum;
				vol = obj.removeTop(vol, deltaZNum);
			}
			else if( idx == 172) {
				deltaZNum = 130;
//				deltaZNum = 140;
...
//			p2.show("projection" + idx);
			...
			saveName = outputPath + saveId + ".tif";
	...
	public void enhanceRegions(Grid3D img, float thresBone, float thresAir, float s)
	{
		float val;
		for(int i = 0; i < img.getSize()[0]; i ++)
			for(int j = 0; j < img.getSize()[1]; j++)
				for(int k = 0; k < img.getSize()[2]; k++) {
					...
					if(val >= thresBone)
						img.setAtIndex(i, j, k, val * s);
					else if(val < thresAir)
						img.setAtIndex(i, j, k, 0);
