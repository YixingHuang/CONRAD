...
import edu.stanford.rsl.Yixing.Celphalometric.spacing.SpacingDataTest;
import edu.stanford.rsl.Yixing.Celphalometric.spacing.SpacingData;
...
public class GenerateTrainingProjections {
...
//	public ConeBeamBackprojector cbbp;
	...
	OpenCLGrid2D sino2DCL;
	
	public static void main(String[] args) throws Exception{
		new ImageJ();
		SpacingData sp = new SpacingData();
		GenerateTrainingProjections obj = new GenerateTrainingProjections();
		String inputPath = "E:\\CQ500CTData\\UpperHalf\\";
		String outputPath = "D:\\Tasks\\FAU4\\TruncationCorrection\\clinical\\CQ500ProjectionsNoisy\\";
		String name, saveName;
		float s = 0.00002f * 1.216f;
		obj.initialGeometry();
...
//				obj.zeroPadding(vol, vol2, deltaZNum);
				vol = obj.zeroPadding(vol, deltaZNum);
...
//			obj.getPerspectiveProjection();
			...
			obj.sinoCL.release();
			obj.addPoissonNoise3DCL(obj.sinogram);
		
			imp = ImageUtil.wrapGrid(obj.sinogram, null);
			saveName = outputPath + "projection" + idx + ".tif";
...
//			obj.sinoCL.release();
	...
	private Grid2D addPoissonNoiseCL(Grid2D sinogram) throws Exception{
		...
		sino2DCL = new OpenCLGrid2D(sinogram);
		sino2DCL.getGridOperator().multiplyBy(sino2DCL, -1.0f);
		sino2DCL.getGridOperator().exp(sino2DCL);
		sino2DCL.getGridOperator().multiplyBy(sino2DCL, photonNumber);
		Grid2D I = new Grid2D(sino2DCL);
		sino2DCL.release();
		
		PoissonNoiseFilteringTool poisson = new PoissonNoiseFilteringTool();
		poisson.applyToolToImage(I);
		sino2DCL = new OpenCLGrid2D(I);
		sino2DCL.getGridOperator().divideBy(sino2DCL, photonNumber);
		sino2DCL.getGridOperator().log(sino2DCL);
		sino2DCL.getGridOperator().multiplyBy(sino2DCL, -1.0f);
		sino2DCL.getGridOperator().removeNegative(sino2DCL);
		Grid2D sino2 = new Grid2D(sino2DCL);
		sino2DCL.release();
		return sino2;
