...
public class CropClinicalSinogram {
	...
	public static void main(String[] args) throws Exception {
		...
		String path = "D:\\Tasks\\FAU1\\Research_LimitedAngleReconstruction\\TVresults\\MagdeburgBrain\\"; //path for wTV data
		String savePath = "D:\\Tasks\\FAU4\\TruncationCorrection\\clinical\\clinicRecons\\";

		CropClinicalSinogram obj = new CropClinicalSinogram(); 
		obj.initialGeometry();
		...
		for(int idx = 1; idx <= 1; idx++){
			...
			nameProj = path + "sinogram.tif";
			imp1 =IJ.openImage(nameProj);
			obj.sinogram = ImageUtil.wrapImagePlus(imp1);
			int[] sz = obj.sinogram.getSize();
			int len = 100;
			Grid3D sinogram2 = new Grid3D(sz[0], sz[1] - len, sz[2]);
			for(int i = 0; i < sz[0]; i++)
				for(int j = 0; j < sz[1] - len; j++)
					for(int k = 0; k < sz[2]; k++)
						sinogram2.setAtIndex(i, j, k, obj.sinogram.getAtIndex(i, j + len/2, k));
						
			
			imp1 = ImageUtil.wrapGrid(sinogram2, null);
			IJ.saveAs(imp1, "Tiff", (savePath + "projectionCrop" + idx+ ".tif"));
...
//			        projMats[i].initFromP(tempMat);
	...
	private void getProjectionMatrix2() throws IOException{
		 String Rawpath="D:\\Tasks\\FAU1\\Research_LimitedAngleReconstruction\\TVresults\\MagdeburgBrain";
		 String Rawname="ProjMatrix.bin";
		
		 boolean isSave = true;
		String outName = "D:\\Tasks\\FAU4\\TruncationCorrection\\clinical\\projMatrxNew.txt";
		BufferedWriter bw = new BufferedWriter(new FileWriter(outName));
...
//			        projMats[i].initFromP(tempMat);
			        if(isSave)
			        {
			        	if(k == 1)
			        	{
			        		val = ProjGrid3D.getSubGrid(i).getAtIndex(k, m) + ProjGrid3D.getSubGrid(i).getAtIndex(k + 1, m ) * dv;
			        		bw.write(String.format("%6.10f", val) + " ");
			        	}
			        	else		
			        		bw.write(String.format("%6.10f", ProjGrid3D.getSubGrid(i).getAtIndex(k, m)) + " ");
			        }
