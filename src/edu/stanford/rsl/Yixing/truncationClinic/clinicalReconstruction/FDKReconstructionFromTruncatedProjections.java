...
public class FDKReconstructionFromTruncatedProjections {
	...
	public static void main(String[] args) throws Exception {
		...
		String path = "D:\\Tasks\\FAU4\\TruncationCorrection\\clinical\\clinicProjections\\"; //path for wTV data
		String savePath = "D:\\Tasks\\FAU4\\TruncationCorrection\\clinical\\clinicRecons\\";

		FDKReconstructionFromTruncatedProjections obj = new FDKReconstructionFromTruncatedProjections(); 
		obj.initialGeometry();
		...
		for(int i = 1; i <= 1; i++){
			...
			obj.sinogram.clone().show("truncated sinogram");
			obj.sinoCL.release();
//			if(isNoisy)
...
//			obj.sinogram.show("sinogram");
			...
			IJ.saveAs(imp1, "Tiff", (savePath + "FDK_NO_WCE" + i+ ".tif"));
...
//			        projMats[i].initFromP(tempMat);
	...
	private void getSparseProjectionMatrix() throws IOException{
		 String Rawpath="D:\\Tasks\\FAU1\\Research_LimitedAngleReconstruction\\TVresults\\MagdeburgBrain";
		 String Rawname="ProjMatrix.bin";
		
		 boolean isSave = true;
		String outName = "D:\\Tasks\\FAU4\\TruncationCorrection\\clinical\\sparseProjMatrx.txt";
		BufferedWriter bw = new BufferedWriter(new FileWriter(outName));
...
//			        projMats[i].initFromP(tempMat);
			        if(isSave)
			        {
			        	if(k == ProjGrid3D.getSize()[0] - 1)
			        		bw.write(String.format("%6.10f", ProjGrid3D.getSubGrid(i).getAtIndex(k, m) * 2) + " ");
			        	else
			        		bw.write(String.format("%6.10f", ProjGrid3D.getSubGrid(i).getAtIndex(k, m)) + " ");
			        }
				}
