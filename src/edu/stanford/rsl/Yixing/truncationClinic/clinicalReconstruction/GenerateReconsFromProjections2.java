...
public class GenerateReconsFromProjections2 {
	...
	public Grid3D sinogram, reconFDK, gt;
	
	public static void main(String[] args) throws Exception {
		new ImageJ();
		
		String path = "D:\\Tasks\\FAU4\\TruncationCorrection\\clinical\\CQ500ReconsNoisy\\"; //path for wTV data
		String pathNoisy = "D:\\Tasks\\FAU4\\TruncationCorrection\\clinical\\CQ500ProjectionsNoisy\\";
		String savePath = "D:\\Tasks\\FAU4\\TruncationCorrection\\clinical\\CQ500ReconsNoisy2\\";

		GenerateReconsFromProjections2 obj = new GenerateReconsFromProjections2(); 
		obj.initialGeometry();
		CLCommandQueue testqueue = OpenCLUtil.getStaticContext().getMaxFlopsDevice().createCommandQueue();
...
//		float s = 1.216f;
		...
		for(int i = 69; i <= 104; i++){
			nameVol = path + "GT" + idx + ".tif";
			projDir=new File(nameVol);
			if (!projDir.exists())
				continue;
			imp1 =IJ.openImage(nameVol);
			obj.gt = ImageUtil.wrapImagePlus(imp1);
			obj.volCL = new OpenCLGrid3D(obj.gt);
			
			obj.cbbp=new ConeBeamBackprojector();
			nameVol = pathNoisy + "projection" + i + ".tif";
			projDir=new File(nameVol);
			if (!projDir.exists())
				continue;
			imp1 =IJ.openImage(nameVol);
			...
			obj.op.truncateProjections(obj.sinoCL, numTrunc);
			...
			for(int projIdx = 0; projIdx < obj.maxProjs; projIdx++)
			{
				...
				obj.sinogram.setSubGrid(projIdx, (Grid2D) tempSino.clone());
			...
			IJ.saveAs(imp1, "Tiff", (savePath + "WCE" + idx + ".tif"));
		
			obj.reconCL.getGridOperator().subtractBy(obj.reconCL, obj.volCL);
			
			imp1 = ImageUtil.wrapGrid(obj.reconCL, null);
			IJ.saveAs(imp1, "Tiff", (savePath + "artifact" + idx+ ".tif"));
			obj.reconCL.release();
			obj.volCL.release();
			System.out.println(testqueue.getContext().getMemoryObjects().size()+ " memory objects");
