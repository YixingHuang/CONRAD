package edu.stanford.rsl.Yixing.truncationClinic.evaluation.gtreferencePix;
...
public class CalculatePsnrFoVForEachPatientPix2pix3 {
	...
	public static void main(String[] args) throws IOException{
		...
		String scalePath = "C:\\Tasks\\FAU4\\TruncationCorrection\\NoiseFree3D\\FDKScalePix\\";
...
//				rmseWce = getPSNR_Full(wce2d, gt2d);
//				rmseWtv = getPSNR_Full(wtv2d, gt2d);
//				rmseUnet = getPSNR_Full(unet2d, gt2d);
//				rmseDcr = getPSNR_Full(dcr2d, gt2d);
				rmsePix = getPSNR_FOV(pix2d, gt2d);
//				rmseDirect = getPSNR_Full(direct2d, gt2d);
				rmseScale = getPSNR_FOV(scale2d, gt2d);
//				rmseSub = getPSNR_Full(sub2d, gt2d);
//				rmseFista = getPSNR_Full(fista2d, gt2d);
				rmseFistaProj = getPSNR_FOV(fistaProj2d, gt2d);
//				rmseFistaWtv = getPSNR_Full(fistaWtv2d, gt2d);
//				rmseFistaProjWtv = getPSNR_Full(fistaProjWtv2d, gt2d);
...
//				bw1.flush();
				if(idx == 3 && i == 174)
					System.out.println(idx + " " + i + ":" + rmsePix +  " " + rmseScale + " "  + rmseFistaProj );
...
//			bw0.close();
		...
		System.out.println("average: " + " " + meanPix/17.0 +  " " + meanScale/17.0 + " " + meanFistaProj/17.0 );
	}
...
//	
	...
	private static double getPSNR_FOV(Grid2D recon_data, Grid2D ref) {
		double maxV = 0;
		double err = 0;
		...
		{
			x = i - (ref.getSize()[0] - 1)/2.0f;
			for(int j = 0; j < ref.getSize()[1]; j++)
			{
				y = j - (ref.getSize()[1] - 1)/2.0f;
				if(x * x + y * y < thres2)
				{
					count ++;
