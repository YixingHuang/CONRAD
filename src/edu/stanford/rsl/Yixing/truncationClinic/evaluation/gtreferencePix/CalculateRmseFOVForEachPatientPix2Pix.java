...
public class CalculateRmseFOVForEachPatientPix2Pix {
...
//			bw1 = new BufferedWriter(new FileWriter(outPutDir1));
			for(int i = 20; i < gt.getSize()[2] - 20; i++) {
				...
				rmsePix = RMSE_FOV(pix2d, gt2d);
				rmseDirect = RMSE_FOV(direct2d, gt2d);
				rmseScale = RMSE_FOV(scale2d, gt2d);
				rmseSub = RMSE_FOV(sub2d, gt2d);
				rmseFista = RMSE_FOV(fista2d, gt2d);
				rmseFistaProj = RMSE_FOV(fistaProj2d, gt2d);
				rmseFistaWtv = RMSE_FOV(fistaWtv2d, gt2d);
				rmseFistaProjWtv = RMSE_FOV(fistaProjWtv2d, gt2d);
