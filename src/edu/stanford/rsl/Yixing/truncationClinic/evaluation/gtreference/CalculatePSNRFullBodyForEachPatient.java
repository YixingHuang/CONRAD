...
public class CalculatePSNRFullBodyForEachPatient {
...
//			bw1 = new BufferedWriter(new FileWriter(outPutDir1));
			for(int i = 20; i < gt.getSize()[2] - 20; i++) {
				...
				rmseFbp = getPSNR_FullBody(fbp2d, gt2d);
				rmseWce = getPSNR_FullBody(wce2d, gt2d);
				rmseWtv = getPSNR_FullBody(wtv2d, gt2d);
				rmseUnet = getPSNR_FullBody(unet2d, gt2d);
				rmseDcr = getPSNR_FullBody(dcr2d, gt2d);
				rmsePix = getPSNR_FullBody(pix2d, gt2d);
				rmseDirect = getPSNR_FullBody(direct2d, gt2d);
				rmseScale = getPSNR_FullBody(scale2d, gt2d);
				rmseSub = getPSNR_FullBody(sub2d, gt2d);
				rmseFista = getPSNR_FullBody(fista2d, gt2d);
				rmseFistaProj = getPSNR_FullBody(fistaProj2d, gt2d);
				rmseFistaWtv = getPSNR_FullBody(fistaWtv2d, gt2d);
				rmseFistaProjWtv = getPSNR_FullBody(fistaProjWtv2d, gt2d);
...
//	}
	...
	private static double getPSNR_FullBody(Grid2D recon_data, Grid2D ref) {
		double err = 0;
		maxV = 0;
