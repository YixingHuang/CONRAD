package edu.stanford.rsl.Yixing.truncationClinic.evaluation.conebeamartifacts;
...
public class CalculateRmseFullForEachPatient {
...
//			bw1 = new BufferedWriter(new FileWriter(outPutDir1));
			for(int i = 20; i < gt.getSize()[2] - 20; i++) {
				...
				rmseFbp = RMSE_Full(fbp2d, gt2d);
				rmseWce = RMSE_Full(wce2d, gt2d);
				rmseWtv = RMSE_Full(wtv2d, gt2d);
				rmseUnet = RMSE_Full(unet2d, gt2d);
				rmseDcr = RMSE_Full(dcr2d, gt2d);
				rmsePix = RMSE_Full(pix2d, gt2d);
				rmseDirect = RMSE_Full(direct2d, gt2d);
				rmseScale = RMSE_Full(scale2d, gt2d);
				rmseSub = RMSE_Full(sub2d, gt2d);
				rmseFista = RMSE_Full(fista2d, gt2d);
				rmseFistaProj = RMSE_Full(fistaProj2d, gt2d);
				rmseFistaWtv = RMSE_Full(fistaWtv2d, gt2d);
				rmseFistaProjWtv = RMSE_Full(fistaProjWtv2d, gt2d);
...
//				if(idx == 9 && i == 122)
//					System.out.println(idx + " " + i + ":" + rmseFbp + " " + rmseWce + " " + rmseWtv + " " + rmseUnet + " " + rmseDcr + " " + rmsePix + " " + rmseDirect + " " + rmseScale + " " + rmseSub + " " + rmseFista + " " + rmseFistaProj + " " + rmseFistaWtv + " " + rmseFistaProjWtv );
				System.out.println(rmseUnet + " " + rmsePix + " " + rmseScale + " " + rmseFistaProj);
			}
//			bw1.close();
//			bw0.close();
...
//	
	...
	private static double RMSE_Full(Grid2D recon, Grid2D recon_data) {
		double err = 0;
		Grid2D temp = new Grid2D(recon);
		temp.getGridOperator().subtractBy(temp, recon_data);
		temp.getGridOperator().multiplyBy(temp, temp);
