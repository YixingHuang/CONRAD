...
public class ConvertTestResultsUNet {
	public static void main(String[] args) throws IOException{
		new ImageJ();
		ConvertTestResultsUNet obj = new ConvertTestResultsUNet();
		String path = "D:\\Tasks\\FAU4\\TruncationCorrection\\NoiseFree3D\\recon\\";
		String savePath = "D:\\Tasks\\FAU4\\TruncationCorrection\\NoiseFree3D\\UNetRecons\\";
		String testPath = "D:\\Tasks\\FAU4\\TruncationCorrection\\NoiseFree3D\\testData_d1\\";
		String testFolder;
		ImagePlus imp1, imp2;
		...
		for(int idx = 1; idx<=18; idx ++){
			...
			for(int i=0; i < data.getSize()[2]; i++) {
				...
				testName = testPath + idx + "\\evaluation\\" + i + "_final_prediction.tif";
				imp2=IJ.openImage(testName);
				test2D = ImageUtil.wrapImagePlus(imp2).getSubGrid(0);
				test.setSubGrid(i, test2D);
			}
			
			test.clone().show("test");
			data.getGridOperator().subtractBy(data, test);
			saveName = savePath + "UNet" + idx + ".tif";
